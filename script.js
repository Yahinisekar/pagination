...
function displayEmployees(start) {
  ...
  let add = `<table class='table table-bordered'><thead><tr><th>ID</th><th>Name</th><th>Email</th></tr></thead><tbody>`;

  for (let i = start; i < start + 10 && employees.length; i++) {
    add += `
    
    
    <tr>
      
       <td id="para">${employees[i].id}</td>
    
    
       
       <td id="name">${employees[i].name}</td>
    
  
       
       <td id="email">${employees[i].email}</td>
    </tr>
